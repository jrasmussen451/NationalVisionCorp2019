four51.app.factory('CustomKitDisplayService', ['$sce', '$451', 'Variant', 'Product', function($sce, $451, Variant, Product){
    ...
    function productViewScope(scope){
        ...
        scope.setAddToOrderErrors = function(){
            ...
            if(scope.allowAddFromVariantList){
                ...
                if(scope.kititem.Product.Type == 'VariableText' && (scope.variantLineItems && !Object.keys(scope.variantLineItems).length))
                    newErrorList.push("Please create a variant.");
                else if(!haveQty && !haveQtyError)
                    newErrorList.push("Please select a quantity");

            }else if(!scope.kititem.Quantity && !scope.kititem.qtyError)//if there's a qty error, just use that. in this case, there's no qty error because it hasn't been validated yet.
                newErrorList.push("Please select a quantity.");

            if(scope.kititem.qtyError)
                newErrorList.push(scope.kititem.qtyError);

            if(!scope.kititem.Variant && scope.kititem.Product.IsVBOSS){
                newErrorList.push("Please select an active product");
            }
            angular.forEach(scope.kititem.Specs, function(s){
        ...
        scope.specChanged = function(spec){
            ...
            if(spec.DefinesVariant)
            {
                ...
                $451.filter(scope.kititem.Specs, {Property: 'DefinesVariant', Value:true}, function(item){
                ...
                if(hasAllVarDefiningSpecs){
                    ...
                    Variant.get({'ProductInteropID': scope.kititem.Product.InteropID, 'SpecOptionIDs': specOptionIDs}, function(data){
                        if(!data.IsDefaultVariant)
                            scope.kititem.Variant = data;
                        newLineItemScope(scope);
                    }, function(ex){
                        scope.kititem.Variant = null;
                    });
                }
            }
        ...
        if(scope.kititem.Variant){
            //scope.kititem.Variant = variant;
            scope.StaticSpecGroups = scope.kititem.Variant.StaticSpecGroups || scope.kititem.Product.StaticSpecGroups;
        }else{
            scope.StaticSpecGroups = scope.kititem.Product.StaticSpecGroups;
        }
    ...
    function newLineItemScope(scope){
        ...
        if(!scope.kititem.Specs){//it's possible we're reloading this due to changing a variant and we don't want to leave the spec values behind
            scope.kititem.Specs = {};
            angular.forEach(scope.kititem.Product.Specs, function(item){
                if(item.CanSetForLineItem || item.DefinesVariant)
                {
                    hasAddToOrderSpecs = true;
                    scope.kititem.Specs[item.Name] = item;// Object.create(item);
                }
            });
        }

        scope.allowAddFromVariantList =
            (scope.kititem.Product.ShowSpecsWithVariantList || !hasAddToOrderSpecs)
            && !scope.kititem.Variant
            && scope.kititem.Product.Variants && scope.kititem.Product.Variants.length > 0
            && ((scope.kititem.Product.Variants && scope.kititem.Product.Variants.length > 0) || scope.kititem.Product.Type == 'VariableText')
        ...
        function determinePriceSchedule() {
            ...
            if (scope.currentOrder && scope.currentOrder.Type == 'Replenishment') {
                if (scope.kititem.Variant && scope.kititem.Variant.ReplenishmentPriceSchedule)
                    return scope.kititem.Variant.ReplenishmentPriceSchedule;
                return scope.kititem.Product.ReplenishmentPriceSchedule;
            }
            else if (scope.currentOrder && scope.currentOrder.Type == 'Standard') {
                if (scope.kititem.Variant && scope.kititem.Variant.StandardPriceSchedule)
                    return scope.kititem.Variant.StandardPriceSchedule;
                return scope.kititem.Product.StandardPriceSchedule;
            }
            else if (scope.kititem.Product.Type == 'VariableText') {
                // the opposite of default ps is true for variable products
                return scope.kititem.Product.ReplenishmentPriceSchedule || scope.kititem.Product.StandardPriceSchedule;
            }
            else {
                if (scope.kititem.Variant && !scope.kititem.Product.IsVBOSS)
                    return scope.kititem.Variant.StandardPriceSchedule || scope.kititem.Variant.ReplenishmentPriceSchedule;
                return scope.kititem.Product.StandardPriceSchedule || scope.kititem.Product.ReplenishmentPriceSchedule
            }

//          return (scope.currentOrder && scope.currentOrder.Type == 'Replenishment') ?
//              (scope.kititem.Variant && scope.kititem.Variant.ReplenishmentPriceSchedule) ? scope.kititem.Variant.ReplenishmentPriceSchedule : scope.kititem.Product.ReplenishmentPriceSchedule :
//              (scope.kititem.Variant && scope.kititem.Variant.StandardPriceSchedule) ? scope.kititem.Variant.StandardPriceSchedule : scope.kititem.Product.StandardPriceSchedule;
        }

        if(scope.kititem.Variant){
            scope.kititem.PriceSchedule = determinePriceSchedule(); //include user permissions to decide to show
            //moved to productViewScope scope.StaticSpecGroups = scope.kititem.Variant.StaticSpecGroups || scope.kititem.Product.StaticSpecGroups;
        }
        else if (!scope.kititem.Product.IsVariantLevelInventory && (scope.currentOrder && scope.currentOrder.Type == 'Replenishment')) {
            scope.kititem.PriceSchedule = scope.kititem.Product.ReplenishmentPriceSchedule;
            scope.allowAddFromVariantList = false;
        }
        else{
            //don't show price schedule if variant overrides parent PS
            scope.kititem.PriceSchedule = variantHasPriceSchedule(scope.kititem.Product, scope.currentOrder ?  scope.currentOrder.Type + 'PriceSchedule' : 'StandardPriceSchedule') ?
                null :
                determinePriceSchedule();
            if(scope.allowAddFromVariantList){
                var p = scope.kititem.Product;
                ...
                angular.forEach(p.Variants, function(v){
                    ...
                    scope.variantLineItems[v.InteropID] = {
                        ...
                        || (scope.currentOrder ? p[scope.currentOrder.Type + 'PriceSchedule'] : (v.StandardPriceSchedule || v.ReplenishmentPriceSchedule || scope.kititem.PriceSchedule)),
                        Product: p,
                        Variant: v,
                        Specs: scope.kititem.Specs
                    };
                });
            }
        }
        ...
        function allowAddToOrder() {
            return (scope.currentOrder ? canAddToOrderType(scope.currentOrder.Type) : true) &&
                (scope.allowAddFromVariantList || (scope.kititem.Variant || (scope.kititem.Product.VariantCount == 0 && scope.kititem.Product.Type != 'VariableText'))
                );
        }
        ...
        function canAddToOrderType(type) {
            ...
            if (!scope.variantLineItems && !scope.kititem.PriceSchedule)
                return false;
            // this is such an outlier that I'm not sure how to weave it into the checks.
            if ((scope.currentOrder && scope.currentOrder.Type == 'Replenishment') && type == 'Replenishment' && !scope.kititem.Product.IsVariantLevelInventory && (scope.kititem.PriceSchedule && scope.kititem.PriceSchedule.OrderType == 'Replenishment')) {
                scope.allowAddToOrder = scope.user.Permissions.contains(type + 'Order'); // even have to reset this
                return true;
            }

            // mpower variants
            if (scope.variantLineItems && scope.kititem.Product.Type == 'VariableText') {
                ...
                return scope.kititem.Product[type + 'PriceSchedule'] != null;
                //return scope.kititem.PriceSchedule.OrderType == type && scope.user.Permissions.contains(type + 'Order');
            }

            return scope.user.Permissions.contains(type + 'Order')
            && scope.variantLineItems ? scope.variantLineItems[scope.kititem.Product.Variants[0].InteropID].Variant[type + 'PriceSchedule'] != null : scope.kititem.Product[type + 'PriceSchedule'] != null
            && (scope.currentOrder && scope.currentOrder.ID ? scope.currentOrder.Type == type : true)
            && (scope.currentOrder && scope.currentOrder.ID ? (scope.variantLineItems ? scope.variantLineItems[scope.kititem.Product.Variants[0].InteropID].PriceSchedule.OrderType : scope.kititem.PriceSchedule.OrderType) == scope.currentOrder.Type : true);
        }

        function canCreateVariant() {
            if (scope.currentOrder && scope.currentOrder.Type == 'Replenishment')
                return false;
            if (scope.kititem.Product.Type == 'VariableText') {
                return true;
            }
            return false;
        }
        ...
        scope.kititem.Product.DisplayInventory = scope.kititem.Product.DisplayInventory && (!scope.currentOrder || (scope.currentOrder && scope.currentOrder.Type == 'Standard'));
        //short view//scope.allowAddToOrder = scope.kititem.Product.Variants.length == 0 && scope.lineItemSpecs.length == 0 && scope.kititem.Product.Type != 'VariableText';
    ...
    return{
        ...
        setProductViewScope: function(scope){
            ...
            //productViewName(scope.kititem.Product);
        },
