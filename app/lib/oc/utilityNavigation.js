                   ...
                   //'<a href="cart">Cart</a>',
                    ...
                    '<li class="user" ng-show="user.Type != \'TempCustomer\'" ng-class="{\'active\': isActive([\'admin\', \'addresses\', \'address\', \'messages\', \'message\', \'favoriteorders\'])}">',
                        ...
                        '<ul class="dropdown-menu">',
                            ...
                            '</a>',
                            '</li>',
                            '<li id="451qa_addy_link" ' +
                            'ng-show="user.Type == \'Customer\' && (user.Permissions.contains(\'CreateShipToAddress\') || user.Permissions.contains(\'CreateBillToAddress\'))" ' +
                            'class="addresses" ng-class="{\'active\': isActive(\'address\'), \'active\': isActive(\'addresses\')}">',
                            '<a href="addresses">',
                            '<i class="fa fa-book"></i>',
                            '{{\'Addresses\' | r | xlat}}',
                            '</a>',
                            '</li>',
                            '<li id="451qa_mesg_link" ng-show="user.Type == \'Customer\' && user.Permissions.contains(\'ViewMessaging\')" class="messages" ng-class="{\'active\': isActive(\'messages\'), \'active\': isActive(\'message\')}">',
                            '<a href="message">',
                            '<i class="fa fa-envelope"></i>',
                            '{{\'Messages\' | r | xlat}}',
                            '</a>',
                            '</li>',
                            '<li class="favorites" ng-show="user.Type == \'Customer\'" ng-class="{\'active\': isActive(\'favoriteorders\')}">',
                            '<a id="451qa_fave_link" href="favoriteorders">',
                            '<i class="fa fa-heart"></i>',
                            '{{\'Favorites\' | r | xlat}}',
                            '</a>',
                            '</li>',
                            '<li class="report mainNavigation-navbar-linkItem" ng-if="user.Type == \'Customer\' && user.Permissions.contains(\'AdvancedReporting\')" ' +
                            'ng-class="{\'active\': isActive(\'reports\')}">',
                            '<a id="451qa_report_link" href="reports">',
                            '<i class="fa fa-bar-chart-o"></i>',
                            '<span class="">{{\'Reports\' | r | xlat}}</span>',
                            '</a>',
                            '</li>',
                            '<li ng-class="{\'active\': isActive(\'contactus\')}">',
                            '<a href="contactus">',
                            '<i class="fa fa-envelope"></i>',
                            '<span class="">{{\'Contact Us\' | r | xlat}}</span>',
                            '</a>',
                            '</li>',
                            '<li ng-class="{\'active\': isActive(\'custominquiry\')}">',
                            '<a href="custominquiry">',
                            '<i class="fa fa fa-pencil-square-o"></i>',
                            '<span class="">{{\'Custom Inquiry\' | r | xlat}}</span>',
                            '</a>',
                            '</li>',
                            '<li ng-class="{\'active\': isActive(\'returntoinv\')}">',
                            '<a href="returntoinv">',
                            '<i class="fa fa fa-pencil-square-o"></i>',
                            '<span class="">{{\'Return to Inventory\' | r | xlat}}</span>',
                            '</a>',
                            '</li>',
