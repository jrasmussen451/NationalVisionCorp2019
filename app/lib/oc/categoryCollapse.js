...
function categorycollapse() {
    ...
    function template() {
        return [
            ...
            '</a>',
            '</li>',
            '<li class="order" ng-if="user.Type == \'Customer\'" ng-class="{\'active\': isActive(\'order\')}">',
            '<a id="451qa_order_link" href="order">',
            '<i class="fa fa-clipboard"></i>',
            '<span>{{\'Orders\' | r | xlat}}</span>',
            '</a>',
            '</li>',
            '<li id="451qa_addy_link" ng-show="user.Type == \'Customer\' && (user.Permissions.contains(\'CreateShipToAddress\') || user.Permissions.contains(\'CreateBillToAddress\'))" ' +
            'class="addresses" ng-class="{\'active\': isActive(\'addresses\')}">',
            '<a href="addresses">',
            '<i class="fa fa-book"></i>',
            '{{\'Addresses\' | r | xlat}}',
            '</a>',
            '</li>',
            '<li id="451qa_mesg_link" ng-show="user.Type == \'Customer\' && user.Permissions.contains(\'ViewMessaging\')" class="messages" ng-class="{\'active\': isActive(\'messages\')}">',
            '<a href="message">',
            ...
            '<li class="451_cat_item" ng-class="{\'active\': node.expanded}" ng-if="node.InteropID !== \'FedExQuickPrint\'">',
                ...
                '<ul ng-if="node.SubCategories.length > 0" class="collapse subL1" collapse="!node.expanded">',
                    '<li ng-repeat="subL1 in node.SubCategories" node="node" current="current">',
                        ...
                        '</a>',
                        '<i ng-hide="subL1.SubCategories" class="fa fa-arrow-circle-right"></i>', //this has to be outside of the a for some reason
